...
class BCBQDataSets(torch.utils.data.Dataset):
    ...
    def __init__(self, data_set='BC', validation_subset=False, conf_threshold=0.4, example_length=64000, allowed_voices='satb',
        ...
        if data_set == 'BC':
            ...
            if one_song:
                #  use BQ 8 for training and BQ 9 for validation
                if validation_subset: self.audio_files = [f for f in self.audio_files if '9_BQ018' in f]
                else: self.audio_files = [f for f in self.audio_files if '8_BQ014' in f]
            else:
...
class CantoriaDataSets(torch.utils.data.Dataset):
    ...
    def __len__(self):
        if self.random_mixes: return 800
        else: return self.n_examples
...
class String(torch.utils.data.Dataset):
    ...
    def __init__(self, data_set='String', 
        ...
        # if validation_subset: self.audio_files = [f for f in self.audio_files if 'Violin2' in f]
        # else: self.audio_files = [f for f in self.audio_files if 'Violin2' not in f]
        
        if validation_subset: self.audio_files
        else: self.audio_files

        
        self.total_audio_length = 95
...
def test_hcqt():
    audio_fpath = "/home/ids/chouteau/umss_f0_regularization/Datasets/BC/mixtures_4_sources/1_BC001_part1_satb.wav"
    pump = create_pump_object()
    ...
    for i in range(5):
        ...
        plt.ylabel("Fréquence (CQT bins)")
        plt.tight_layout()
        fig.set_size_inches(w=6.718, h=4)
        plt.savefig(f"figures/hcqt_mag_{i}.pdf", dpi=600)
        ...
        plt.tight_layout()
        fig.set_size_inches(w=6.718, h=4)
        plt.savefig(f"figures/hcqt_dphase_{i}.pdf", dpi=600)
